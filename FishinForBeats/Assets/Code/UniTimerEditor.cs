...
namespace DigitalPenguin.Unity.Timer
{
    ...
    /// </summary>
    [CustomEditor(typeof(UniTimer))]
    [CanEditMultipleObjects]
    public class UniTimerEditor : Editor
    {

        //Add the ShowOnEnum methods in here
        private void SetFieldCondition()
        {
            ShowOnEnum("action", "LOADSCENE", "sceneName");

            ShowOnEnum("action", "TOGGLE_GAMEOBJECT", "activate");
            ShowOnEnum("action", "TOGGLE_GAMEOBJECT", "toggleObject");

            ShowOnEnum("action", "ANIMATIONTRIGGER", "animationTriggerName");
            ShowOnEnum("action", "ANIMATIONTRIGGER", "repeating");

            ShowOnEnum("action", "UNITY_EVENT", "unityEvent");
            ShowOnEnum("action", "UNITY_EVENT", "repeating");
        }
            ...
            if (obj.NextVisible(true))
            {
                ...
                do
                {
                    ...
                    foreach (var fieldCondition in fieldConditions)
                    {
                        ...
                        else if (FiledNameMatches(obj, fieldCondition))
                        {
                            ...
                            if (EnumValueMismatch(fieldCondition, currentEnumValue))
                            {
                                // there may be another condition ahead which allows this field to be shown
                                p_FieldCondition another = fieldConditions.Find(c => c != fieldCondition && FiledNameMatches(obj, c) && !EnumValueMismatch(c, currentEnumValue));
                                shouldBeVisible = another != null;
                                break;
                            }
                        }
                    }
        ...
        private static bool FiledNameMatches(SerializedProperty obj, p_FieldCondition fieldCondition)
        {
            return fieldCondition.p_fieldName == obj.name;
        }

        private static bool EnumValueMismatch(p_FieldCondition fieldCondition, object currentEnumValue)
        {
            return currentEnumValue.ToString() != fieldCondition.p_enumValue;
        }
